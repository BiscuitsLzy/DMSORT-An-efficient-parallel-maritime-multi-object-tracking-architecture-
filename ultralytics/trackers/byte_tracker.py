...
def _nn_res_recons_cosine_distance(x, y, tmp=100, data_is_normalized=False):
    ...
    ftrk = torch.from_numpy(np.asarray(x)).float().cuda()
    fdet = torch.from_numpy(np.asarray(y)).float().cuda()
    # #对特征进行增强(到时候放到把两个特征融合以后，别放到前面）(到时候试试Ledoit-Wolf 协方差估计
    #具体是这个（cov_estimator = LedoitWolf().fit(data_weighted)
    #sigma_reg = cov_estimator.covariance_）
    # wide_matrix=torch.mm(ftrk.transpose(0, 1), ftrk)/(ftrk.shape[0]-1)
    # wide_soft = F.softmax(tmp * wide_matrix, dim=0)#这个最好让他没有（把他改成dim为0的归一化就行（使用在合并特征以后用l2范数归一化512维度的feature）
    # ftrk = torch.mm(wide_soft, ftrk.transpose(0, 1))
    ...
    #（现在是直接把得分平均，也可以先求出来之后的特征加权相加后求相似度）（特征增强应该放在这里）
...
class BYTETracker:
    ...
    def update(self, results, img=None):
        ...
        # Start thread1 to compute warp = self.gmc.apply(img, dets)
        thread1 = threading.Thread(target=self.calculate_warp, args=(img, dets))
        thread1.start()

        # Start thread2 to compute detections_second = self.init_track(dets_second, scores_second, cls_second, img)
        thread2 = threading.Thread(target=self.calculate_detections_second,
                                   args=(dets_second, scores_second, cls_second, img))
        thread2.start()

        # Main thread: Compute detections
        ...
        # Get results from thread1 and thread2(
        with self.lock:
            warp = self._thread1_warp_result
            detections_second = self._thread2_detections_second_result
        ...
        if hasattr(self, "gmc") and warp is not None:
        ...
        if len(self.removed_stracks) > 1000:
            self.removed_stracks = self.removed_stracks[-999:]  # Clip to 1000 removed tracks maximum

        return np.asarray([x.result for x in self.tracked_stracks if x.is_activated], dtype=np.float32)

    def calculate_warp(self, img, dets):
        """Thread to compute warp = self.gmc.apply(img, dets)"""
        if hasattr(self, "gmc") and img is not None:
            warp = self.gmc.apply(img, dets)
            with self.lock:
                self._thread1_warp_result = warp

    def calculate_detections_second(self, dets_second, scores_second, cls_second, img):
        """Thread to compute detections_second = self.init_track(dets_second, scores_second, cls_second, img)"""
        detections_second = self.init_track(dets_second, scores_second, cls_second, img)
        with self.lock:
            self._thread2_detections_second_result = detections_second
    def calculate_matches_1(self, strack_pool, detections, match_thresh):
        """Thread for computing the first set of matches."""
        dists = self.get_dists(strack_pool, detections)
        matches, u_track, u_detection = matching.linear_assignment(dists, thresh=match_thresh)
        with self.lock:
            self._thread1_results = (matches, u_track, u_detection)

    def calculate_matches_2(self, r_tracked_stracks, detections_second):
        """Thread for computing the second set of matches."""
        dists = matching.iou_distance(r_tracked_stracks, detections_second)
        matches, u_track, u_detection_second = matching.linear_assignment(dists, thresh=0.5)
        with self.lock:
            self._thread2_results = (matches, u_track, u_detection_second)
